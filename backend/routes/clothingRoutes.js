...
// Crear producto de indumentaria
...
router.get('/', async (req, res) => {
  try {
    const products = await ClothingProduct.find().sort({ createdAt: -1 });
    res.json(products);
  } catch (error) {
...
router.put('/:id', async (req, res) => {
  try {
    const updated = await ClothingProduct.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
      runValidators: true
    });
    res.json(updated);
